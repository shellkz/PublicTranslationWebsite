...
function renderEditionRow(e) {
  ...
  const metaParts = [e.date].filter(Boolean).join(' · ');
  return `<div class="edition-row">
      <span class="edition-lang">${escapeHtml(e.language)}</span>
      <a href="${escapeHtml(e.url)}">${escapeHtml(linkLabel)}</a>
      <span class="edition-meta">${escapeHtml(metaParts)}${metaParts ? ' · ' : ''}${escapeHtml(e.copyrightStatus)}</span>
    </div>`;
}
...
function renderTranslationRow(t) {
  ...
  // 譯者、中間譯者名稱在這裡只顯示純文字,要點進去看譯者頁請從別處(如 /translators/ 列表)點。
  const via = t.sourceTranslatorName ? `,經 ${escapeHtml(t.sourceTranslatorName)} 譯本轉譯` : '';
  return `<a class="entry-row" href="${escapeHtml(t.url)}">
      <div class="entry-title">${escapeHtml(t.title)}</div>
      <div class="entry-meta">譯者・${escapeHtml(t.translatorId)}(依據語言:${escapeHtml(t.editionLanguage)}${via})</div>
...
<section class="block">
  <div class="block-head">
    <div class="block-title serif">已知來源版本 <span class="jp">Editions</span></div>
  </div>
...
<section class="block">
  <div class="block-head">
    <div class="block-title serif">站內譯本 <span class="jp">Translations</span>(${vm.translations.length})</div>
  </div>
  ${translationsSection}
</section>
`;
